 ...
 */
 ...
 export class CrmPage {
	...
	filter()
	{
		...
		const modal = this.modalCtrl.create('FilterCrmPage', passingData);
	...
	next()
	{
		const modal = this.modalCtrl.create('NextCrmPage');
	...
	linkFeedback(token:string)
	{
		// console.log(this.env.base_url+"edigindo/crm/feedback?token="+token);
		this.clipboard.copy(this.env.base_url+"edigindo/crm/feedback?token="+token);

		this.util.presentToast('Link has been copy to clipboard!');
	}
